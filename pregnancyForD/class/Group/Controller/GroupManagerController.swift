    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        cell.longPressBlock = {[unowned self](content)in
            ...
            let okAction = UIAlertAction(title: "好的", style: .default, handler: {[unowned self](action)->() in
                ...
                HttpRequestManager.shareIntance.tagsOperation(tagId: (data?.id?.intValue)!, tagname: (data?.tagName)!, opsType: 2, callback: { [unowned self](success, array, message) in
                    if success == true{
                        self.requestData()
...
extension GroupManagerController {
    ...
    func requestData(){
        ...
        HttpRequestManager.shareIntance.tagsOperation(tagId: 0, tagname: "", opsType: 0) { [unowned self](success, array, message) in
            if success == true{
    ...
    func modifyGroup(_ value : NSInteger, groupS : String, callback : @escaping (_ success : Bool, _ message : String)->()){
        ...
        if patienId != nil {
            HttpRequestManager.shareIntance.updatePatientInfo(patientId: (patienId?.intValue)!, type: 0, value: value) { [unowned self](success, message) in
