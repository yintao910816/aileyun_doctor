...
extension ConsultModule : UITableViewDataSource, UITableViewDelegate{
    ...
    func doctorReject(contId : NSNumber){
        SVProgressHUD.show()
        HttpRequestManager.shareIntance.HC_updateConsultStatus(consultId: contId.intValue, doctorId: (UserManager.shareIntance.currentUser?.id?.intValue)!) {[unowned self](isLock) in
            ...
            let okAction = UIAlertAction(title: "确认", style: .default, handler: {[unowned self](action)->() in
                HttpRequestManager.shareIntance.doctorReject(consultationId: contId.intValue) { (success, message) in
                    if success == true {
    ...
    func reply(contId : NSNumber){
        SVProgressHUD.show()
        HttpRequestManager.shareIntance.HC_updateConsultStatus(consultId: contId.intValue, doctorId: (UserManager.shareIntance.currentUser?.id?.intValue)!) {[weak self](isLock) in
            guard isLock == true else {
                SVProgressHUD.dismiss()
                showAlert(title: "提醒", message: "小组成员正在回复中,请稍等...")
                return
