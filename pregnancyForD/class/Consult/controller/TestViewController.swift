...
//        patientM.allowEdit = doctorId?.intValue == UserManager.shareIntance.currentUser?.id?.intValue ? true : false
    ...
    func requestData(){    //张洁
        ...
        queue.async(group: group, qos: .default, flags: []) {
            //请求患者资料H5地址
            HttpRequestManager.shareIntance.HC_getH5URL(keyCode: "PATIENT_RECORDS_2017") { [weak self](success, info) in
                if success == true {
                    self?.url = info
                }else{
        ...
        if let pId = patientId, let docId = doctorId{
            ...
            queue.async(group: group, qos: .default, flags: [], execute: {
                HttpRequestManager.shareIntance.getPatientInfo(patientId: pId.intValue, doctorId: docId.intValue) { [weak self](success, model, message) in
                    if success == true {
    ...
    func requestReplyDetail(isRefresh : Bool){
        ...
        if let pId = patientId{
            ...
            SVProgressHUD.show()
            HttpRequestManager.shareIntance.HC_findPatientReplys(patientId: pId.intValue, doctorIds: s, pageNum: 0, pageSize: 10) { [weak self](success, models, hasNext, message) in
                if success == true {
                    SVProgressHUD.dismiss()
                    self?.pageNo += 1
...
//        let testURL = "http://192.168.0.102:8088/examples/d/reproduction/view/cycleList.html"
