...
extension PhotoPickerView {
    ...
    @objc func sendAction(){
        ...
        DispatchQueue.global().async {[unowned self]()in
            ...
            HttpRequestManager.shareIntance.uploadImage(doctorId: doctorId, imageArr: imageArr, type: "3", consultationId: consultID!) { [weak self](success, message) in
                if success == true {
...
extension PhotoPickerView : UINavigationControllerDelegate, UIImagePickerControllerDelegate {
    func imagePickerController(_ picker: UIImagePickerController, didFinishPickingMediaWithInfo info: [String : Any]) {
        ...
        SVProgressHUD.show()
        HttpRequestManager.shareIntance.uploadImage(doctorId: doctorId, imageArr: [image], type: "3", consultationId: i!) { [weak self](success, message) in
            if success == true {
                HCShowInfo(info: "图片发送成功！")
                self?.removeFromSuperview()
                self?.clearSelectedStatus()
