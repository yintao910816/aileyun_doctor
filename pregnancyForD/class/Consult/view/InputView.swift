...
//    weak var contVC : ConsultViewController?
    ...
    lazy var voiceConfirmView : ConfirmVoiceView = {
        ...
        confirV.sendBlock = {[weak self]()in
            ...
            // 发送语音
            SVProgressHUD.show()
            HttpRequestManager.shareIntance.uploadVoice(doctorId: (self?.doctorId!)!, voiceFile: (self?.amrPath())!, type: "3", consultationId: (self?.consultId?.intValue)!) {(success, message) in
                if success == true {
...
extension InputView : UITextViewDelegate{
    ...
    func textView(_ textView: UITextView, shouldChangeTextIn range: NSRange, replacementText text: String) -> Bool {
        if text.compare("\n") == ComparisonResult.orderedSame {
            ...
            if textView.text != "" {
                HCPrint(message: "开始回复")
                SVProgressHUD.show()
                HttpRequestManager.shareIntance.reply(doctorId: self.doctorId!, consultationId: (consultId?.intValue)!, content: textView.text, urlList: "", type: "0", callback: { [weak self](success, message) in
                    if success == true {
