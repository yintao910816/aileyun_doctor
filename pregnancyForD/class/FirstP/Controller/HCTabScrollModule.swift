...
class HCTabScrollModule: NSObject {
    ...
    func gradualChange(p : CGFloat){
        ...
        //按钮颜色
        let firstC = UIColor.init(red: 254/255.0 * (1-p), green: 127/255.0 * (1-p), blue: 146/255.0 * (1-p), alpha: 1)
        let secondC = UIColor.init(red: 254/255.0 * p, green: 127/255.0 * p, blue: 146/255.0 * p, alpha: 1)
        for b in btnArr{
            if b.tag == 0{
