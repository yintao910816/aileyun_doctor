...
class FirstPageViewController: BaseViewController {    //  谭丽华
    ...
    lazy var scrollV : UIScrollView = {
        ...
        s.backgroundColor = kLightGrayColor
        return s
    }()
    ...
    @objc func requestData(){
        ...
        group.enter()
        HttpRequestManager.shareIntance.HC_bannerList() { [weak self](bool, arr) in
            if bool == true{
                self?.picScrollV.dataArr = arr
            }else{
                HCShowInfo(info: "banner未能获取数据")
            }
            group.leave()
        }
        
        group.enter()
        HttpRequestManager.shareIntance.HC_functionList() { [weak self](bool, arr) in
            if bool == true{
                ...
                HCShowInfo(info: "functionList未能获取数据")
            }
            group.leave()
        }
        
        group.enter()
        HttpRequestManager.shareIntance.HC_dynamicList(pageNum: 1, pageSize: 10) { [weak self](bool, consultArr, notiArr) in
            if bool == true{
                self?.consultM.modelArr = consultArr
                self?.consultM.notiArr = notiArr
            }else{
                HCShowInfo(info: "dynamicList未能获取数据")
                self?.consultM.modelArr = [ConsultMessageModel]()
                self?.consultM.notiArr = [NotificationModel]()
            }
            group.leave()
        }
        
        group.enter()
        HttpRequestManager.shareIntance.HC_getH5URL(keyCode: "NOTICE_DETAIL_URL_DOCTOR") { [weak self](success, info) in
            if success == true {
