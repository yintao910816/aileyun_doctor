...
extension EditViewController {
    ...
    @objc func confirmAction(){
        if inputTextView.text != "" {
            ...
            if isEditGroup == true{
                HttpRequestManager.shareIntance.tagsOperation(tagId: tagid!, tagname: inputTextView.text, opsType: 1, callback: { (success, array, message) in
                    if success == true{
            ...
            if isAddTemplate == true{
                ...
                HttpRequestManager.shareIntance.consultTemplate(templateValue: 0, templateContent: inputTextView.text, opsType: 3) { [unowned self](success, array, message) in
...
extension EditViewController {
    func addTagGroup(groupName : String, callback : @escaping (_ success : Bool, _ message : String)->()){
        HttpRequestManager.shareIntance.tagsOperation(tagId: 0, tagname: groupName, opsType: 3) { (success, array, message) in
    ...
    func updateUserInfo(key : String, value : String, callback : @escaping (_ success : Bool, _ message : String)->()){
        ...
        HttpRequestManager.shareIntance.updateinfo(infoDic: infoDic as NSDictionary) { (success, message) in
            callback(success, message)
        }
    }
