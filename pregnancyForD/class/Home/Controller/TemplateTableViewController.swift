...
//    weak var inputVC : InputViewController?
    ...
    override func tableView(_ tableView: UITableView, commit editingStyle: UITableViewCellEditingStyle, forRowAt indexPath: IndexPath) {
        if editingStyle == UITableViewCellEditingStyle.delete {
            ...
            HttpRequestManager.shareIntance.consultTemplate(templateValue: (tableViewArray[indexPath.row].id?.intValue)!, templateContent: "", opsType: 2) { [unowned self](success, array, message) in
...
extension TemplateTableViewController {
    ...
    func requestData(){
        HttpRequestManager.shareIntance.consultTemplate(templateValue: 0, templateContent: "", opsType: 0) { [unowned self](success, array, message) in
            if success == true {
