...
extension UserTableViewController {
    ...
    func userSetting(){
        ...
        settingVC.shareBlock = {[unowned self]()in
    ...
    func initFooterView(){
        let footContainerV = UIView.init(frame: CGRect.init(x: 0, y: 0, width: SCREEN_WIDTH, height: 150))
        ...
        versionL.snp.updateConstraints { (make) in
            make.top.equalTo(logoutBtn.snp.bottom).offset(15)
...
extension UserTableViewController {
    func requestData(){
        ...
        queue.async(group: group, qos: .default, flags: []) {
            //请求患者资料H5地址
            HttpRequestManager.shareIntance.HC_getH5URL(keyCode: "DOCTOT_BILL") { [weak self](success, info) in
                if success == true {
                    self?.billURL = info
        ...
        queue.async(group: group, qos: .default, flags: []) {
            HttpRequestManager.shareIntance.getUserInfo() { [unowned self](success, model, message) in
                if success == true {
